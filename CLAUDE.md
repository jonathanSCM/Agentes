...
El modelo de lenguaje **nunca** decide qué productos existen, cuáles mostrar, ni inventa datos (precio, stock, disponibilidad, moneda, fotos). Todo eso lo calcula el código en `lib/services/catalogo.js` (`buscarConFallback`, `paginar`, `fotoParaMostrar`) y se lo entrega ya resuelto a `lib/services/agente.js`. Tampoco puede afirmar que hizo algo (mandar una foto, el QR, la ubicación) si el envío no devolvió éxito real: toda tool responde `TOOL_SUCCESS` o `TOOL_FAILED`. Esto está reforzado con validaciones en código (no solo instrucciones de texto) — ver `docs/03-decisiones-recientes.md` para ejemplos de bugs reales que pasaron cuando esto no se cumplía.
...
5. **Verificar antes de afirmar que algo funciona.** Los tests (`npm test`, ~105 tests con `node:test`) y simulaciones con `llamarInyectado` (inyectar una IA falsa determinista) son el patrón para probar el motor del agente sin depender de una API real. Ver ejemplos en `test/regresion-agente.test.js`, `test/motor-busqueda.test.js` y `test/documento-12-casos.test.js` (los 12 casos que pide el documento del dueño, uno por uno). Todos son deterministas salvo `regresion-agente.test.js`, que necesita una base de datos real.
6. **La regla de oro del producto** (documento del dueño): primero escuchar, después preguntar, después recordar, después consultar, después filtrar, después mostrar, después ayudar a decidir, después cerrar. Si el sistema no lo sabe, la IA tampoco lo sabe; si el sistema no confirmó que una acción ocurrió, la IA no puede decir que ocurrió.
