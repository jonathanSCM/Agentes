...
//
// Bug real probando en vivo: un link tipo "https://maps.google.com/?q=lat,lng"
// (dominio "maps.google.com", MUY comun - lo generan varias apps de GPS y es
// el formato de toda la vida de "compartir ubicacion" en desktop) no
// matcheaba ninguno de los patrones (solo se cubria "google.com/maps", que es
// un dominio distinto) - el link se guardaba como texto suelto sin resolver,
// y recien en confirmar_pedido el cliente se enteraba de que "la ubicacion
// no es valida", despues de que el bot ya le habia seguido preguntando el
// nombre como si la hubiese aceptado.
function extraerUrlDeMaps(texto) {
  const m = String(texto || '').match(/https?:\/\/(maps\.app\.goo\.gl|goo\.gl\/maps|(?:www\.)?google\.com\/maps|maps\.google\.com)\S*/i);
  return m ? m[0] : null;
}
