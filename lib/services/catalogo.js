 ...
 * WhatsApp (ver mostrar_tarjeta_categoria en agente.js): cuantos productos
 * hay con stock, desde que precio, y hasta 2 modelos reales para nombrar.
 * Nunca inventa nada - si no hay stock, cantidad=0 y todo lo demas viene
 * vacio/null, para que el codigo que llama decida no mandar la tarjeta.
 *
 * "Destacados" prioriza el mas barato (primera impresion de precio) y
 * despues el de mas stock (mas seguro de que siga disponible), sin
 * repetir - es una heuristica simple y explicable, no una recomendacion
 * "inteligente".
 */
function resumenCategoria(productos = [], categoriaId) {
  const conStock = productos.filter((p) => p.categoriaId === categoriaId && tieneStock(p));
  if (!conStock.length) return { cantidad: 0, precioDesde: null, destacados: [] };

  const precioDesde = Math.min(...conStock.map((p) => Number(p.precio)));
  const masBarato = [...conStock].sort((a, b) => Number(a.precio) - Number(b.precio))[0];
  const stockDe = (p) => {
    const activas = (p.variantes || []).filter((v) => v.activa);
    return activas.length ? activas.reduce((s, v) => s + (v.stock || 0), 0) : p.stock;
  };
  const masStock = [...conStock].sort((a, b) => stockDe(b) - stockDe(a)).find((p) => p.id !== masBarato.id);

  // Foto candidata para la tarjeta cuando la tienda no cargo una imagen de
  // categoria a mano: la del producto mas barato, o la del de mas stock si
  // el mas barato no tiene ninguna. Nunca inventa una URL - si ninguno de
  // los dos tiene fotos, queda null y el que llama decide (ver
  // mostrar_tarjeta_categoria en agente.js).
  const fotoDestacada = (masBarato.fotos && masBarato.fotos[0])
    || (masStock && masStock.fotos && masStock.fotos[0])
    || null;

  return {
    cantidad: conStock.length,
    precioDesde,
    destacados: [masBarato, masStock].filter(Boolean).map((p) => p.nombre),
    fotoDestacada,
  };
}
...
// Para el catalogo web: un chip por cada color distinto del producto, con su
// foto representativa (la primera variante activa de ese color que tenga
// fotos propias). Si ningun color repite, no tiene sentido mostrar la
// seccion - eso lo decide quien la use (largo <= 1).
function coloresConFotoDeVariantes(p) {
  const porColor = new Map();
  for (const v of (p.variantes || []).filter((v) => v.activa)) {
    const color = v.atributos?.Color;
    if (!color || porColor.has(color)) continue;
    porColor.set(color, v.fotos?.length ? v.fotos[0] : null);
  }
  return [...porColor.entries()].map(([color, foto]) => ({ color, foto }));
}

/**
 * Mensaje prellenado (formato WhatsApp) para el link "wa.me" del catalogo
 * web: la idea es que el cliente vuelva a WhatsApp con un texto ya armado y
 * prolijo en vez de tener que describir de nuevo lo que quiere. A proposito
 * no es un formato rigido de un solo caso - sirve tanto para "solo estoy
 * mirando" (sin variante ni cantidad) como para "esto es justo lo que quiero
 * comprar" (con variante y cantidad), y para cualquier boton nuevo que en el
 * futuro necesite mandar al cliente de vuelta a WhatsApp con contexto.
 *
 * @param {object} producto  al menos { nombre }.
 * @param {object} [opciones]
 * @param {object|null} [opciones.variante]  variante elegida, si aplica ({ atributos }).
 * @param {number|null} [opciones.cantidad]  si es mas de 1, se antepone "Nx".
 * @param {string} [opciones.accion]  verbo inicial ("Estoy viendo", "Quiero", etc.).
 */
function mensajeWhatsappProducto(producto, opciones = {}) {
  const { variante = null, cantidad = null, accion = 'Estoy viendo' } = opciones || {};
  const nombre = (producto && producto.nombre) || '';
  const prefijoCantidad = cantidad && cantidad > 1 ? `${cantidad}x ` : '';
  const detalle = variante && variante.atributos
    ? Object.entries(variante.atributos).map(([k, v]) => `${k}: ${v}`).join(', ')
    : '';
  return `${accion} *${prefijoCantidad}${nombre}*${detalle ? ` (${detalle})` : ''}`;
}
...
module.exports = {
  ...
  buscarProductosFiltrados, buscarConFallback, paginar, fotoParaMostrar, coloresDeVariantes, coloresConFotoDeVariantes,
