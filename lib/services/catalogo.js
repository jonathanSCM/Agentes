...
function coincideAtributosLead(p, lead = {}) {
  ...
  for (const [nombre, valor] of Object.entries(atributos)) {
    ...
    if (!coincideTexto(String(valor), valorProducto)) return false;
  }
  return true;
}
 ...
 *
 * Solo aparecen rubros y subcategorias que tienen productos con stock: un menu
 * de venta no puede ofrecer una puerta que no lleva a ningun lado.
 *
 * @returns {Array<{id, nombre, orden, productos, subcategorias: Array}>}
 */
function arbolDeCategorias(productos = []) {
...
function subcategoriasDe(productos, categoriaId) {
  const rubro = arbolDeCategorias(productos).find((r) => r.id === categoriaId);
