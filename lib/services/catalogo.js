...
function fotoParaMostrar(p, lead = {}) {
  ...
  if (coloresPedidos.length) {
    ...
    const delColor = variantes.filter((v) => coloresPedidos.some((c) => normalizarSimple(v.atributos?.Color).includes(c)));
    if (delColor.length) {
    ...
    if (otra) {
      return { ...base, url: otra.fotos[0], urls: [...otra.fotos], colorDeLaFoto: otra.atributos?.Color || null, esDelColorPedido: false };
    }
    return {
      ...base,
