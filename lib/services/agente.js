...
// Texto libre de un producto (nombre + caracteristicas + descripcion + los
// atributos de sus variantes activas: talla, color, etc.) para poder matchear
// marca/talla/color contra lo que el cliente pidio, ya que esos datos no
// siempre son un campo estructurado propio del producto.
function textoCompletoProducto(p) {
  const atributosVariantes = (p.variantes || [])
    .filter((v) => v.activa)
    .map((v) => Object.values(v.atributos || {}).join(' '))
    .join(' ');
  return `${p.nombre} ${(p.caracteristicas || []).join(' ')} ${p.descripcion || ''} ${atributosVariantes}`;
}
...
function construirSystem(empresa, productos, config, lead = {}, sinHerramientas = false, esPrimerMensaje = false) {
...
${esPrimerMensaje ? `ESTE ES EL PRIMER MENSAJE DE LA CONVERSACION - TOMA LA INICIATIVA (obligatorio): no respondas solo con algo generico tipo "¿en que puedo ayudarte?". Presentate brevemente: quien sos, de que tienda, que vende la tienda en general (mira las categorias reales de abajo), y como podes ayudar. Ejemplo del tono esperado (adaptalo, no lo copies literal):
"¡Hola! Soy el asistente de ventas de ${marca}. Tenemos ${categoriasReales.length ? categoriasReales.slice(0, 3).join(', ') : 'varios productos'}. Te puedo ayudar a encontrar justo lo que buscas. ¿Que andas necesitando?"
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      const system = construirSystem(empresa, productos, agente.config, leadActual, esUltimaVuelta, esPrimerMensaje);
