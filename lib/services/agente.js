...
function afirmaFaltaDeStockSinRespaldo(texto) {
  ...
  return /(no (tenemos|hay|contamos con|disponemos de)\b[^,.?!]{0,25}(stock|disponible|disponibilidad|en existencia))/.test(t);
}
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        if (!esUltimaVuelta) {
        ...
        } else if (pidiendoPreferencia || nombrado || listadoEnTexto || interrogatorio || anuncioDeBusqueda || faltaStockFalsa || llamadaEnTexto) {
          ...
          logEtapa('texto_vago_forzado_en_ultima_vuelta', telefonoCliente, { vuelta, pidiendoPreferencia, nombrado, listadoEnTexto, interrogatorio, anuncioDeBusqueda, faltaStockFalsa, llamadaEnTexto, yaEnCierre });
...
module.exports = {
  ...
  afirmaFaltaDeStockSinRespaldo, pareceQuererCatalogoCompleto, pareceLlamadaDeHerramientaEnTexto,
};
