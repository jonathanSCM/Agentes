...
const {
  ...
  valoresRealesDeAtributo, resolverValorDeAtributo,
 ...
 * (talla/color/marca) y despues atributosLead, con el matching tolerante.
 */
function leadYaTiene(lead = {}, nombre) {
  const campoPropio = ATRIBUTO_A_CAMPO_PROPIO[String(nombre).toLowerCase()];
  if (campoPropio) return Boolean(lead[campoPropio]);
  return Boolean(valorEnAtributosLead(lead.atributosLead, nombre));
}
