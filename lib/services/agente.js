  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        // FORZADO EN CODIGO: nombro un producto real en el texto pero no
        // mando ninguna tarjeta en este turno. El cliente lee "tenemos las
        // Park St 2.0" y no ve nada: ni foto, ni precio, ni tallas. Todo dato
        // de producto va en tarjeta (bug real reportado con capturas).
        if (!esUltimaVuelta && !helpers.tarjetasEnTurno && candidatosActuales.length) {
          const nombrado = nombraUnProductoReal(resp.content, candidatosActuales);
          if (nombrado) {
            logEtapa('producto_nombrado_sin_tarjeta', telefonoCliente, { producto: nombrado });
            mensajesTurno.push({ role: 'assistant', content: resp.content });
            mensajesTurno.push({ role: 'user', content: `RECORDATORIO DEL SISTEMA (esto no lo dijo el cliente): nombraste "${nombrado}" en tu texto pero NO mandaste su tarjeta, asi que el cliente lee el nombre y no ve ni la foto, ni el precio, ni las tallas. Llama AHORA a mostrar_productos con ese producto. Si ya se lo mostraste antes en la conversacion, mandaselo igual: puede haber pasado un rato largo y no lo tiene a mano.` });
            continue;
          }
        }
