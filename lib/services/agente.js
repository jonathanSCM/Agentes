...
function afirmaQueMostroAlgoSinTarjeta(texto) {
  if (!texto) return false;
  const t = normalizarSimple(texto);
  return /(te muestro|aqui (tienes|puedes revisar|te dejo|las tienes|van los|tenes)|te paso|te envie|te comparto|echale un vistazo|hechale un vistazo|dale un vistazo|revisa las opciones|mira las opciones|dejame mostrarte|aca (tienes|van)|estas son las opciones)/.test(t);
}
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        if (!esUltimaVuelta) {
        ...
        } else if (pidiendoPreferencia || nombrado || listadoEnTexto || interrogatorio || anuncioDeBusqueda || faltaStockFalsa || llamadaEnTexto || afirmoSinMostrar) {
          ...
          logEtapa('texto_vago_forzado_en_ultima_vuelta', telefonoCliente, { vuelta, pidiendoPreferencia, nombrado, listadoEnTexto, interrogatorio, anuncioDeBusqueda, faltaStockFalsa, llamadaEnTexto, afirmoSinMostrar, yaEnCierre });
