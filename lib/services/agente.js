...
function afirmaFalloTecnicoInventado(texto) {
  if (!texto) return false;
  const t = normalizarSimple(texto);
  return /(hubo un (inconveniente|problema|error)|parece que hubo|intento (mandartelas?|enviartelas?|mostrartelas?|mandarlas?|enviarlas?|mostrarlas?|pasartelas?) de nuevo|no se pudo (enviar|mandar|mostrar))/.test(t);
}
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        if (!esUltimaVuelta) {
        ...
        } else if (pidiendoPreferencia || nombrado || listadoEnTexto || interrogatorio || anuncioDeBusqueda || faltaStockFalsa || llamadaEnTexto || afirmoSinMostrar || debioLlamarTarjetaCategoria || falloInventado) {
          ...
          logEtapa('texto_vago_forzado_en_ultima_vuelta', telefonoCliente, { vuelta, pidiendoPreferencia, nombrado, listadoEnTexto, interrogatorio, anuncioDeBusqueda, faltaStockFalsa, llamadaEnTexto, afirmoSinMostrar, debioLlamarTarjetaCategoria, falloInventado, yaEnCierre });
...
module.exports = {
  ...
  afirmaFaltaDeStockSinRespaldo, afirmaQueMostroAlgoSinTarjeta, afirmaFalloTecnicoInventado, pareceQuererCatalogoCompleto, pareceQuererVerOtraCosa, pareceLlamadaDeHerramientaEnTexto,
};
