...
function formatearProductos(productos) {
  return productos
    .map((p) => {
      ...
      const base = `- [ID ${p.id}] ${p.nombre} - Categoria: ${p.categoria || 'General'} - Precio: ${Number(p.precio).toFixed(2)} - Stock: ${p.stock}${atrib}${caract} - ${p.descripcion || ''}`;
...
async function ejecutarFuncion(toolCall, contexto, helpers) {
  const { empresaId, telefonoCliente, conexion, fotosParaMostrar, conversacionId, leadActual } = helpers;
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
  ...
  const helpers = { empresaId: empresa.id, empresaSlug: empresa.slug, telefonoCliente, conexion: agente.conexion, fotosParaMostrar, conversacionId, baseUrl: opciones.baseUrl };
  const mensajesTurno = [];
  const MAX_VUELTAS = 3;
