...
function afirmaQueMostroAlgoSinTarjeta(texto) {
  if (!texto) return false;
  const t = normalizarSimple(texto);
  return /(te muestro|aqui (tienes|puedes revisar|te dejo|las tienes)|te paso|te envie|te comparto|echale un vistazo|hechale un vistazo|dale un vistazo|revisa las opciones|mira las opciones)/.test(t);
}
