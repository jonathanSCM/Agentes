...
function fichaProducto(p, lead = {}, moneda = 'BOB') {
  ...
  if (variantes.length) {
    ...
    if (coloresPedidos.length) {
      filtradas = filtradas.filter((v) => coloresPedidos.some((c) => valoresEquivalentes(c, v.atributos?.Color || '')));
    }
...
- SI HAY PRODUCTOS EN EL BLOQUE DE RESULTADOS, MOSTRALOS YA. No pidas color, talla, marca ni presupuesto antes de que el cliente haya visto una sola opcion: eso lo agota y se va. Primero las tarjetas, despues afinas con lo que el te diga.
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        if (!esUltimaVuelta && !helpers.tarjetasEnTurno && candidatosActuales.length && pidePreferenciaSinMostrar(resp.content)) {
