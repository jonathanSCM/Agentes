...
function partesDelSystem(empresa, productos, config, lead = {}, sinHerramientas = false, esPrimerMensaje = false, nombreAgente = '', tarjetaEnviadaEnTurno = false, pedidosPrevios = []) {
...
Preguntaselo en una sola frase y volve a llamar agregar_al_carrito con el idVariante.`;
    ...
    const yaEstabaCerrando = ESTADOS_DE_CIERRE.has(clienteFinal.estadoConversacion);
    ...
    const relacionados = yaEstabaCerrando ? [] : productosRelacionados(contexto, producto, idsEnCarrito);
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
  ...
  if (lead.contexto && lead.contexto.carritoWebPendiente) {
    ...
    if (productoAgregado) {
      ...
      const relacionados = ESTADOS_DE_CIERRE.has(lead.estadoConversacion)
        ? []
        : productosRelacionados(productos, productoAgregado, idsEnCarrito);
      const bloqueRelacionados = relacionados.length
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      const { fijo: systemFijo, variable: systemVariable } = partesDelSystem(empresa, productos, agente.config, leadActual, esUltimaVuelta, esPrimerMensaje, agente.nombre, Boolean(helpers.tarjetasEnTurno), pedidosPrevios);
