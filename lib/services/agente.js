...
function seccionProductos(productos, lead = {}, categoria = null, moneda = 'BOB', config = null) {
  ...
  const inicialesFaltantes = preguntasInicialesFaltantes(config, lead);
...
En ESTE mensaje preguntá UNA sola cosa: "${inicialesFaltantes[0]}"${inicialesFaltantes.length > 1 ? ' (lo que falte se lo preguntas despues, de a uno)' : ''}. Hacelo natural, como lo preguntaria un vendedor ("¿es para vos o para regalar?", "¿buscas algo de hombre o de mujer?"), NUNCA como un formulario.
Guardalo con actualizar_datos_lead (en atributosCategoria si no es talla/color/marca). Recien cuando lo sepas se te habilita el catalogo.`;
  }
 ...
 */
function preguntasInicialesFaltantes(config, lead = {}) {
  ...
  return pedidas.filter((nombre) => {
    ...
    return campoPropio ? !lead[campoPropio] : !atributosLead[nombre];
  });
}
...
async function ejecutarFuncion(toolCall, contexto, helpers) {
  ...
  if (toolCall.name === 'mostrar_productos' || toolCall.name === 'enviar_fotos_producto' || toolCall.name === 'ver_mas_productos') {
    ...
    if (toolCall.name !== 'enviar_fotos_producto' && leadActual) {
      const faltanIniciales = preguntasInicialesFaltantes(config, leadActual);
...
Preguntaselo en una sola frase y volve a llamar agregar_al_carrito con el idVariante.`;
    ...
    const relacionados = contexto
      .filter((p) => p.id !== producto.id && p.categoriaId === producto.categoriaId && !idsEnCarrito.has(p.id) && tieneStock(p))
      .slice(0, 3);
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
  ...
  const detectados = extraerFiltros(textoParaExtraccion, productos, clienteFinal.categoriaId);
  ...
  // color, nombre, direccion) se conserva: esa memoria si le sirve.
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        // FORZADO EN CODIGO: hay productos listos para mostrar y el modelo,
        // en vez de mostrarlos, le pide al cliente color/talla/marca. Se
        // rechaza y se le exige mostrar primero.
        // Si el cliente ya eligio un producto, preguntarle la talla NO es
        // filtrar: es completar la compra. Ahi si corresponde.
        const yaEligio = Boolean(lead.productoFavoritoId);
        if (!esUltimaVuelta && !helpers.tarjetasEnTurno && !yaEligio && candidatosActuales.length && pidePreferenciaSinMostrar(resp.content)) {
        ...
        // FORZADO EN CODIGO: nombro un producto real, o escribio un listado
        // con precio, en el texto - pero no mando ninguna tarjeta en este
        // turno. El cliente lee "tenemos las Park St 2.0" (o una lista con
        // precios) y no ve nada: ni foto, ni precio real, ni tallas. Todo
        // dato de producto va en tarjeta (bug real reportado con capturas).
        //
        // Pero SOLO mientras el cliente todavia esta explorando: si ya eligio
        // (favorito o algo en el carrito) y esta en pleno cierre (direccion,
        // forma de pago, confirmar_pedido), es NORMAL que el texto nombre el
        // producto ("tus Park St 2.0 llegan a esa direccion") - eso no es
        // "esconder" el producto, y forzar una tarjeta ahi vuelve a mandar
        // la ficha en vez de seguir con el cierre. Bucle real en produccion:
        // el cliente pedia confirmar la direccion y el bot le reenviaba la
        // tarjeta una y otra vez sin nunca llegar a crear_pedido.
        const yaEnCierre = Boolean(lead.productoFavoritoId) || carritoDe(leadActual.contexto || {}, conversacionId).length > 0;
        if (!helpers.tarjetasEnTurno && !yaEnCierre && candidatosActuales.length) {
          const nombrado = nombraUnProductoReal(resp.content, candidatosActuales);
          const listadoEnTexto = pareceListadoDeProductosEnTexto(resp.content);
          if (nombrado || listadoEnTexto) {
            ...
            // (mejor: el modelo elige que decir alrededor de la tarjeta). Si
            // ya se quedo sin vueltas (insistio 2 veces con texto plano), NO
            // se le manda ese texto al cliente sin tarjeta: se fuerza la
            // tarjeta real en codigo, directo, sin depender de que el modelo
            // por fin obedezca. Bug real: sin esto, la ultima vuelta salta
            // TODAS las validaciones (tools:[] ahi) y el texto malo se manda
            // igual - es lo que paso en produccion con "Modelo Urban Street".
            if (!esUltimaVuelta) {
              mensajesTurno.push({ role: 'assistant', content: resp.content });
            ...
            const idsForzados = candidatosActuales.slice(0, MAX_PRODUCTOS_A_MOSTRAR).map((p) => p.id);
            ...
            return { ok: true, demo: false, proveedor, modelo, respuesta: 'Estas son las opciones que tenemos 👆 ¿Alguna te interesa?', fotos: fotosParaMostrar };
          }
        }
