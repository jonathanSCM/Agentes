    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          items: {
            type: 'array',
            description: 'Opcional: si no lo mandas, se usa lo que el cliente tiene en el carrito, que es lo normal.',
            items: {
              type: 'object',
              properties: {
                idProducto: { type: 'integer' },
...
function seccionProductos(productos, lead = {}, categoria = null, moneda = 'BOB', config = null) {
  ...
  if (inicialesFaltantes.length) {
    ...
    return `TODAVIA NO PODES MOSTRAR NADA (ni productos, ni el menu de rubros): esta tienda necesita saber primero ${inicialesFaltantes.join(' y ')}.
...
function linkCatalogoWeb(helpers, ruta) {
  ...
  try {
    ...
    return `${helpers.baseUrl}/catalogo/${helpers.empresaSlug}${ruta}${ruta.includes('?') ? '&' : '?'}s=${encodeURIComponent(token)}`;
  } catch {
    return null;
  }
}
...
// Otros productos reales de la MISMA categoria, con stock, que todavia no
// estan en el carrito - para cross-sell. Nunca inventa: si no hay nada real
// que ofrecer, devuelve una lista vacia.
function productosRelacionados(contexto, producto, idsAExcluir = new Set()) {
  return contexto
    .filter((p) => p.id !== producto.id && p.categoriaId === producto.categoriaId && !idsAExcluir.has(p.id) && tieneStock(p))
    .slice(0, 3);
}
...
function construirSystem(empresa, productos, config, lead = {}, sinHerramientas = false, esPrimerMensaje = false, nombreAgente = '', tarjetaEnviadaEnTurno = false) {
...
EL RECORRIDO DE UNA VENTA (este es el orden acordado con el negocio, seguilo):
 ...
 2. Elige una (por numero, por nombre o diciendo "me interesa") -> le mostras las TARJETAS de esa categoria, hasta 3.
 3. Si pide mas, llamas a ver_mas_productos y le mandas 3 mas, hasta agotar el stock.
 4. El cliente elige mirando las tarjetas ("me interesa la Ginger Tav en blanco talla 9"): la talla y el color ya los vio ahi, NO se los preguntes.
 5. Llamas a agregar_al_carrito y le preguntas SI DESEA VER ALGO MAS.
      - Si dice que si -> volves al paso 1 (mostrar_categorias).
      - Si dice que no -> cerras: nombre, tipo de entrega (y direccion si es a domicilio), forma de pago.
 6. Llamas a confirmar_pedido, le leas el resumen, y con su "si" llamas a crear_pedido.
...
- Cuando el cliente NOMBRA o ELIGE una categoria puntual por primera vez en la conversacion (ej. "estoy buscando zapatillas deportivas", o elige "Zapatillas" de un menu), llama a mostrar_tarjeta_categoria ANTES de mostrar productos individuales - le da una foto y un panorama real de esa categoria (cuantos hay, desde que precio, algunos modelos, y un link para ver todo). Es UNA vez por categoria: si ya se la mostraste en esta conversacion, no la repitas, anda directo a mostrar_productos si pide ver modelos.
...
${seccionProductos(productos, lead, categoriaActual, moneda, config)}
  ...
  if (toolCall.name === 'mostrar_productos' || toolCall.name === 'enviar_fotos_producto' || toolCall.name === 'ver_mas_productos') {
    ...
    if (toolCall.name !== 'enviar_fotos_producto') {
      if (productos.length === 1) {
        linkWeb = linkCatalogoWeb(helpers, `/producto/${productos[0].id}`);
        ...
        linkWeb = linkCatalogoWeb(helpers, `?${params.toString()}`);
      }
    }
...
Preguntaselo en una sola frase y volve a llamar agregar_al_carrito con el idVariante.`;
    ...
    // Cross-sell: otros productos reales de la MISMA categoria, con stock,
    // que todavia no estan en el carrito. Es opcional (el modelo decide si
    // lo menciona o no), pero le da algo real para ofrecer en vez de
    // inventar "tambien tenemos accesorios" sin saber si existen.
    const idsEnCarrito = new Set(items.map((i) => i.productoId));
    const relacionados = productosRelacionados(contexto, producto, idsEnCarrito);
    const bloqueRelacionados = relacionados.length
      ? `\n\nTambien hay stock de esto en la misma categoria (real, por si le sirve ofrecerlo - NUNCA insistas si el cliente no muestra interes):\n${relacionados.map((p) => `- [ID ${p.id}] ${p.nombre} - ${formatearPrecio(p.precio, moneda)}`).join('\n')}`
      : '';

    return `TOOL_SUCCESS: agregado al carrito.
${resumenCarrito(items, precioTexto)}${bloqueRelacionados}

Tu texto ahora: confirmale en una linea que lo agregaste y preguntale SI DESEA VER ALGO MAS.${relacionados.length ? ' Si te parece natural, mencionale UNA de las opciones relacionadas de arriba (con su ID real, por si la pide) - sin forzarlo, es solo una sugerencia.' : ''} Si dice que si quiere ver mas, llama a mostrar_categorias (o a mostrar_productos con el ID sugerido si eligio esa). Si dice que no, cerra: pedile nombre, como quiere recibirlo y forma de pago, y despues llama a confirmar_pedido.`;
  }
  ...
  if (toolCall.name === 'confirmar_pedido' || toolCall.name === 'crear_pedido') {
    ...
    // Por defecto se cobra lo que hay en el CARRITO: es la fuente de verdad.
    // Que el modelo tenga que repetir la lista de items en cada llamada es
    // como se perdian productos cuando el cliente compraba mas de uno.
    const itemsPedidos = (args.items && args.items.length)
      ? args.items
      : itemsParaPedido(carritoDe(clienteFinal.contexto || {}, conversacionId));
...
Tu texto ahora: leele este resumen al cliente de forma clara y ordenada y preguntale si esta todo correcto. NO llames a crear_pedido en este turno: espera a que el cliente confirme que si.`;
  ...
  if (toolCall.name === 'mostrar_categorias') {
    const faltanIniciales = preguntasInicialesFaltantes(config, leadActual || {}, contexto);
    if (faltanIniciales.length) {
      logEtapa('menu_bloqueado_preguntas_iniciales', telefonoCliente, { faltan: faltanIniciales });
      return `TODAVIA NO le muestres el menu (TOOL_FAILED): esta tienda necesita saber primero ${faltanIniciales.join(' y ')}. Preguntá SOLO "${faltanIniciales[0]}" en este mensaje, de forma natural, y guardalo con actualizar_datos_lead. Despues si te doy el catalogo.`;
    }
    ...
    if (leadActual && arbol.length === 1 && !arbol[0].subcategorias.length) {
      ...
      const busqueda = buscarConFallback(contexto, leadActual);
      const idsAMostrar = paginar(busqueda.resultados, (leadActual.contexto || {}).fotosEnviadas || []).pagina.map((p) => p.id);
      if (idsAMostrar.length) {
        await ejecutarFuncion({ name: 'mostrar_productos', arguments: { idsProductos: idsAMostrar } }, contexto, helpers);
        logEtapa('menu_rubro_unico_auto', telefonoCliente, { rubro: unico.nombre, productosMostrados: idsAMostrar.length });
        return `TOOL_SUCCESS. Esta tienda vende solo "${unico.nombre}"; ya se le mostraron sus productos reales arriba 👆 (no hacia falta preguntarle si queria verla, era la unica opcion que hay). Tu texto ahora: preguntale con naturalidad si alguna le interesa.`;
      }
      logEtapa('menu_rubro_unico_auto', telefonoCliente, { rubro: unico.nombre, sinStockQueCalce: true });
      return `TOOL_SUCCESS. Esta tienda vende solo "${unico.nombre}" pero ahora mismo no hay ningun producto con stock que calce con lo que ya sabemos del cliente. Decilo con transparencia, sin inventar nada.`;
    }
  ...
  if (toolCall.name === 'mostrar_tarjeta_categoria') {
    ...
    if (yaMostradas.includes(categoriaActual.id)) {
      return `Ya le mandaste la tarjeta de "${categoriaActual.nombre}" antes en esta conversacion (TOOL_FAILED a proposito, no se repite). Si quiere ver modelos puntuales, segui con mostrar_productos.`;
    }
    ...
    const linkWeb = linkCatalogoWeb(helpers, `?${params.toString()}`);
    ...
    // real lo confirmo. Sin imagen cargada, se manda el mismo texto sin
    // foto - nunca se inventa una imagen que el dueño no subio.
    let envioOk = true;
    if (categoriaActual.imagenUrl && conexion && conexion.estado === 'CONECTADO') {
      const envio = await wa.enviarImagen(conexion, telefonoCliente, categoriaActual.imagenUrl, caption);
      envioOk = envio.ok;
    } else if (conexion && conexion.estado === 'CONECTADO') {
      const envio = await wa.enviarTexto(conexion, telefonoCliente, caption);
      envioOk = envio.ok;
    } else if (categoriaActual.imagenUrl && fotosParaMostrar) {
      ...
      fotosParaMostrar.push({ url: categoriaActual.imagenUrl, caption });
    }
    ...
    logEtapa('tarjeta_categoria_enviada', telefonoCliente, { categoria: categoriaActual.nombre, cantidad: resumen.cantidad, conImagen: Boolean(categoriaActual.imagenUrl) });
    return `TOOL_SUCCESS: se le mando la tarjeta real de "${categoriaActual.nombre}" (${categoriaActual.imagenUrl ? 'con foto' : 'sin foto, esta categoria todavia no tiene una cargada'}).\nTu texto ahora: UNA linea corta y natural invitandolo a mirarla. NO repitas los datos (cantidad, precio, modelos, link) que ya estan en la tarjeta.`;
  }
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
  ...
  if (lead.contexto && lead.contexto.carritoWebPendiente) {
    ...
    if (productoAgregado) {
      const itemsCarritoActual = carritoDe(lead.contexto, conversacionId);
      const idsEnCarrito = new Set(itemsCarritoActual.map((i) => i.productoId));
      const relacionados = productosRelacionados(productos, productoAgregado, idsEnCarrito);
      const bloqueRelacionados = relacionados.length
        ? `\n\nTambien hay stock de esto en la misma categoria (real, por si le sirve ofrecerlo - NUNCA insistas si el cliente no muestra interes):\n${relacionados.map((p) => `- [ID ${p.id}] ${p.nombre} - ${formatearPrecio(p.precio, empresa.moneda)}`).join('\n')}`
        : '';
      avisoCarritoWeb = `RECORDATORIO DEL SISTEMA (esto no lo dijo el cliente en este mensaje - paso hace un momento en el catalogo web): el cliente ACABA de agregar esto a su carrito desde la web:\n${fichaProducto(productoAgregado, lead, empresa.moneda)}${bloqueRelacionados}\n\nTu texto AHORA: saludalo con entusiasmo natural por esa eleccion ("¡buena eleccion!" o similar), confirmale que ya quedo en su carrito, y si hay relacionados arriba sugerí UNO sin insistir. Despues preguntale si quiere ver algo mas. NO le preguntes que producto eligio: ya lo sabes.`;
    }
    try {
      await prisma.clienteFinal.update({
        where: { id: clienteFinal.id },
  ...
  const turno = conversacionId
    ...
    : historial.length;
  const helpers = { empresaId: empresa.id, empresaSlug: empresa.slug, telefonoCliente, conexion: agente.conexion, fotosParaMostrar, conversacionId, baseUrl: opciones.baseUrl, config: agente.config, moneda: empresa.moneda || 'BOB', turno };
  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      const system = construirSystem(empresa, productos, agente.config, leadActual, esUltimaVuelta, esPrimerMensaje, agente.nombre, Boolean(helpers.tarjetasEnTurno));
      ...
      if (!resp.tool_calls?.length) {
        ...
        const yaEnCierre = Boolean(lead.productoFavoritoId) || carritoDe(leadActual.contexto || {}, conversacionId).length > 0;
        ...
        if (!esUltimaVuelta) {
        ...
        } else if (pidiendoPreferencia || nombrado || listadoEnTexto || interrogatorio || anuncioDeBusqueda) {
          ...
          logEtapa('texto_vago_forzado_en_ultima_vuelta', telefonoCliente, { vuelta, pidiendoPreferencia, nombrado, listadoEnTexto, interrogatorio, anuncioDeBusqueda });
          const idsNombrados = helpers.idsPedidosPorNombre || [];
          if (candidatosActuales.length || idsNombrados.length) {
            // Si el cliente nombro un producto, el rescate muestra ESE, no
            // tres candidatos cualquiera de la categoria: preguntó por uno.
            const idsForzados = idsNombrados.length
              ? idsNombrados.slice(0, MAX_PRODUCTOS_A_MOSTRAR)
              : candidatosActuales.slice(0, MAX_PRODUCTOS_A_MOSTRAR).map((p) => p.id);
