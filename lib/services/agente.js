...
function formatearProductos(productos) {
  return productos
    .map((p) => {
      ...
      return `- [ID ${p.id}] ${p.nombre} - Categoria: ${p.categoria || 'General'} - Precio: ${Number(p.precio).toFixed(2)} - Stock: ${p.stock}${caract} - ${p.descripcion || ''}`;
    })
    .join('\n');
}
...
- CASI SIN EMOJIS: la gran mayoria de tus mensajes NO llevan ningun emoji, igual que un vendedor real escribiendo rapido por WhatsApp. Como mucho, uno (nunca mas de uno) de forma muy ocasional en un momento que realmente lo amerite (un cierre con calidez, una buena noticia). Si dudas, no le pongas ninguno: es preferible de menos que de mas.
...
- Los menus numerados (1, 2, 3) son una HERRAMIENTA OCASIONAL, no la forma por defecto de hablar: usalos solo cuando de verdad ayudan a elegir entre 3 o mas opciones concretas que ya mostraste. Para una pregunta abierta, preguntala como la haria una persona ("¿que andas buscando?", "¿es para vos o es un regalo?"), nunca como un formulario con opciones numeradas.

COMO ARRANCA LA CONVERSACION: si el cliente recien saluda o escribe algo generico ("hola", "buenas"), saluda con calidez y preguntale de forma abierta y natural que anda buscando (variá la frase, no uses siempre la misma). NUNCA le tires de entrada una lista de categorias para que elija una por numero: eso es un menu de telefono, no una conversacion. Dejá que el cliente cuente con sus propias palabras que necesita, y a partir de ahi vos entendes a que categoria corresponde (para eso esta el catalogo real de abajo).
...
COMO CONDUCIS LA VENTA (sin que se note el mecanismo):
- Entendé que necesita, mostrale opciones reales, resolvé sus dudas, y cuando muestre interes de verdad avanzá hacia cerrar el pedido. Es un ida y vuelta natural, no una lista de pasos que hay que tildar.
...
- Cuando el cliente muestra interes real en un producto especifico, mostrale fotos con enviar_fotos_producto sin que tengas que pedirlas.
- Si no le convence lo que le mostraste, ofrecele otra alternativa real del catalogo, como lo haria un vendedor que conoce bien su tienda.
...
// ============================ Ejecucion de tools ============================
// helpers: { empresaId, telefonoCliente, conexion, fotosParaMostrar }
async function ejecutarFuncion(toolCall, contexto, helpers) {
  const { empresaId, telefonoCliente, conexion, fotosParaMostrar } = helpers;
  ...
  if (toolCall.name === 'mostrar_productos' || toolCall.name === 'enviar_fotos_producto') {
    ...
    for (const producto of productos) {
      ...
      if (conexion && conexion.estado === 'CONECTADO' && producto.fotos?.length) {
      ...
      } else if (producto.fotos?.length && fotosParaMostrar) {
        ...
        fotosParaMostrar.push({ url: producto.fotos[0], caption });
      }
  ...
  if (toolCall.name === 'crear_pedido') {
    ...
    for (const item of args.items || []) {
      const producto = await prisma.producto.findFirst({ where: { id: Number(item.idProducto), empresaId } });
      ...
      items.push({ productoId: producto.id, nombre: producto.nombre, precio: producto.precio, cantidad });
    }
 ...
 */
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente) {
    ...
    include: {
      ...
      empresa: {
        include: {
          ...
          productos: { where: { activo: true }, orderBy: { nombre: 'asc' }, take: 200 },
        },
      },
    },
  });
  ...
  const helpers = { empresaId: empresa.id, telefonoCliente, conexion: agente.conexion, fotosParaMostrar };
  const mensajesTurno = [];
  const MAX_VUELTAS = 3;
