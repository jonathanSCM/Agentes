...
// Techo de seguridad, no una regla de negocio: el bot debe ofrecer TODO lo
// que aplica realmente (nunca ocultar inventario real por prolijidad), este
// numero solo evita mandar una cantidad absurda de tarjetas si algun dia una
// categoria tiene muchisimos productos. Bien por encima de cualquier
// categoria real del catalogo actual.
const MAX_PRODUCTOS_A_MOSTRAR = 20;
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Presenta productos al cliente como tarjetas visuales (foto + ficha con precio, stock y detalles). SIEMPRE que vayas a mostrar uno o mas productos del bloque de resultados, llama esta funcion con sus IDs en vez de describirlos en texto. Mostra TODOS los productos reales que apliquen a lo que pidio el cliente (si hay 8 que calzan, llama con los 8 IDs) - nunca elijas mostrar solo una parte y calles el resto, eso es ocultar inventario real que si existe.',
      parameters: {
        type: 'object',
        properties: {
          idsProductos: { type: 'array', items: { type: 'integer' }, description: 'IDs exactos de TODOS los productos del bloque de resultados que apliquen, no una seleccion parcial.' },
        },
  ...
  },
  {
    ...
    function: {
      name: 'crear_pedido',
      description: 'Crea el pedido cuando el cliente confirmo que quiere comprar uno o mas productos especificos, YA con nombre, direccion y forma de pago guardados via actualizar_datos_lead. El sistema valida el stock antes de confirmar. Registra la forma de pago elegida pero no procesa ningun cobro: eso lo coordina un asesor humano por fuera del bot.',
      parameters: {
        type: 'object',
        properties: {
          items: {
...
function buscarProductosFiltrados(productos, lead = {}, { ignorarPresupuesto = false, ignorarCategoria = false, ignorarTalla = false } = {}) {
  ...
  return productos
    ...
    .slice(0, MAX_PRODUCTOS_A_MOSTRAR);
}
...
function extraerFiltros(texto, productos = []) {
  ...
  for (const p of productos) {
    if (p.categoria) categoriasUnicas.set(p.categoria.nombre, p.categoria.id);
  }
...
function formatearProductos(productos) {
  return productos
    .map((p) => {
      ...
      const base = `- [ID ${p.id}] ${p.nombre} - Categoria: ${p.categoria?.nombre || 'General'} - Precio: ${Number(p.precio).toFixed(2)} - Stock: ${p.stock}${atrib}${caract} - ${p.descripcion || ''}`;
...
// La MISMA lista que seccionProductos le muestra al modelo en el prompt
// (primero exactos, si no hay relaja presupuesto, si no relaja talla). Se
// usa tambien en ejecutarFuncion para FORZAR en codigo que el modelo muestre
// todos estos productos con tarjeta - no alcanza con pedirselo por texto en
// el prompt, la IA a veces solo llama mostrar_productos con uno y describe
// el resto en su mensaje, rompiendo la regla de "todo va en tarjeta".
//
// IMPORTANTE: la categoria NUNCA se relaja aca. Si el cliente busca zapatillas
// y no hay ninguna que calce, el sistema no ofrece productos de otra
// categoria por su cuenta (ver seccionProductos) - eso violaria la regla de
// negocio de no saltar de categoria automaticamente.
function productosCandidatosAMostrar(productos, lead = {}) {
  if (!filtrosCompletos(lead)) return [];
  ...
  if (lead.presupuesto) {
    ...
    if (sinPresupuesto.length) return sinPresupuesto;
  }
  if (lead.talla) {
    const sinTalla = buscarProductosFiltrados(productos, lead, { ignorarPresupuesto: true, ignorarTalla: true });
    if (sinTalla.length) return sinTalla;
  }
  return [];
}
...
function seccionProductos(productos, lead = {}) {
  ...
  if (exactos.length) {
    ...
    const sinMostrar = exactos.filter((p) => !yaEnviadas.includes(p.id));

    if (!sinMostrar.length) {
      // Ya se mostraron TODOS los resultados reales de esta busqueda en esta
      // misma conversacion. Si se le sigue diciendo "muestraselos" el modelo
      // vuelve a llamar mostrar_productos con lo mismo y reenvia tarjetas
      // duplicadas (esto paso en produccion). Ahora se le avisa explicito.
      return `Resultados de la busqueda (los mismos de antes, YA SE LOS MOSTRASTE a este cliente en esta conversacion - mira el historial, ya tienen tarjeta con foto):\n${formatearProductos(exactos)}\n\nNO vuelvas a llamar mostrar_productos con estos IDs, ya los tiene. Estas son TODAS las opciones reales que hay en esta categoria/talla/presupuesto por ahora, no hay mas para mostrar. Si el cliente pregunta "que mas tienes" o similar, decile con naturalidad que esas son las opciones que hay, preguntale cual le gusto o si quiere que ajustes algo (otro presupuesto, otra talla, otra categoria).`;
    }

    return `Resultados de la busqueda (ya filtrados por categoria, talla y presupuesto pedidos, maximo 3, son los UNICOS productos reales que puedes mencionar, NUNCA inventes otros):\n${formatearProductos(sinMostrar)}\n\nMuestraselos DE INMEDIATO llamando a mostrar_productos UNA SOLA VEZ, con los IDs de TODOS los productos de esta lista juntos en el mismo array (no solo el primero) - el sistema manda una tarjeta con foto y ficha por cada uno. PROHIBIDO nombrar, describir o comparar en tu texto ningun producto de esta lista (ni "tambien tenemos...", ni resumirlos): si esta en la lista, va en la tarjeta, nunca en tu texto. Tu texto de esta vuelta es SOLO una reaccion corta + pregunta de cierre.\n\nADVERTENCIA SOBRE EL HISTORIAL: si en mensajes anteriores dijiste que "no habia productos disponibles" en esta categoria, eso quedo OBSOLETO. El bloque de arriba es la UNICA verdad actual del inventario. Corrigete con naturalidad y muestra los productos.`;
  }

  if (lead.presupuesto) {
    const sinPresupuesto = buscarProductosFiltrados(productos, lead, { ignorarPresupuesto: true });
    if (sinPresupuesto.length) {
      return `No hay nada dentro del presupuesto (${lead.presupuesto}) en esa categoria/talla, PERO si hay estas opciones reales apenas por encima (maximo 3, no inventes otras):\n${formatearProductos(sinPresupuesto)}\n\nNO las muestres todavia con mostrar_productos. Primero PREGUNTALE al cliente si quiere verlas (ej: "No encontre nada exacto dentro de tu presupuesto, pero tengo opciones entre Bs X y Bs Y, ¿queres que te las muestre?"). Revisa tu propio mensaje anterior en el historial: si ya le preguntaste esto y el cliente respondio que si (o algo afirmativo), RECIEN AHI llama a mostrar_productos con estas opciones.`;
    }
  }
  ...
  if (lead.talla) {
    const sinTalla = buscarProductosFiltrados(productos, lead, { ignorarPresupuesto: true, ignorarTalla: true });
    if (sinTalla.length) {
      return `No hay nada en la talla "${lead.talla}" dentro de esta categoria, PERO si hay estas opciones reales en otras tallas (maximo 3, no inventes otras):\n${formatearProductos(sinTalla)}\n\nNO las muestres todavia con mostrar_productos. Primero PREGUNTALE si le sirve otra talla o si quiere verlas igual. Revisa tu mensaje anterior: si ya preguntaste y el cliente confirmo, RECIEN AHI llama a mostrar_productos.`;
    }
  }

  return `NINGUN producto de la categoria "${lead.categoriaInteres}" calza, ni relajando presupuesto o talla. PROHIBIDO ofrecer productos de otra categoria por tu cuenta (nunca saltes de categoria automaticamente, aunque coincidan en presupuesto). Se transparente: decile que por ahora no tenes eso exacto en esa categoria, ofrece derivar_a_asesor para que le avisen cuando haya reposicion, o pregunta si le interesa ver otra categoria distinta (solo si EL lo pide, vos no se la ofrezcas de prepo).`;
}
...
// Que foto mandar: si el cliente ya dijo que color busca y ese color tiene
// fotos propias cargadas en alguna variante (ver Variante.fotos), se manda
// esa - no la generica del producto. La talla no influye aca a proposito:
// las fotos se comparten entre tallas de un mismo color por diseño (ver
// carga por rango de tallas en /panel/productos/:id/variantes).
function fotoParaMostrar(p, lead = {}) {
  ...
  if (coloresPedidos.length) {
    ...
    if (match) return match.fotos[0];
  }
  return p.fotos?.length ? p.fotos[0] : null;
}
...
// Atributos de nivel producto que la categoria del cliente marca como
// obligatorios y todavia no se saben (ni como campo propio del lead -
// talla/color/marca - ni guardados en atributosLead). Los de nivel variante
// (esDeVariante) NO se piden aca a proposito (ver plan: son guia, no
// bloqueo) - solo influyen en que se sugiere al cargar variantes en el panel.
const ATRIBUTO_A_CAMPO_PROPIO = { talla: 'talla', color: 'color', marca: 'marca' };
function atributosFaltantes(categoria, lead = {}) {
  if (!categoria || !categoria.atributos) return [];
  const atributosLead = lead.atributosLead || {};
  return categoria.atributos
    .filter((a) => a.obligatorio && !a.esDeVariante)
    .filter((a) => {
...
function construirSystem(empresa, productos, config, lead = {}, sinHerramientas = false, esPrimerMensaje = false, nombreAgente = '') {
  ...
  const categoriasReales = [...new Set(productos.map((p) => p.categoria?.nombre).filter(Boolean))];
  ...
  const categoriaActual = lead.categoriaId ? productos.find((p) => p.categoria?.id === lead.categoriaId)?.categoria : null;
  const faltantes = atributosFaltantes(categoriaActual, lead);
...
- PREGUNTAS COMERCIALES DE A UNA: cuando necesites entender mejor que busca (talla, color, marca, presupuesto, si es para el o para regalar, que caracteristica le importa mas), preguntalo de forma natural y de a UNA pregunta por mensaje, nunca como una lista de requisitos o un formulario. Dejá que la conversacion fluya.
...
SI EL CLIENTE PREGUNTA QUE TENES / QUE VENDES / QUE PRODUCTOS TIENEN (o algo generico similar, sin especificar un producto puntual): esto es OBLIGATORIO, no opcional.${categoriasReales.length <= 1 ? ` Esta tienda vende UN SOLO rubro (${categoriasReales[0] || marca}), asi que JAMAS respondas con una lista numerada de 1 sola categoria ni preguntes "¿cual te interesa?" (no hay entre que elegir). En vez de eso, decile en UNA frase natural que rubro vendes, mencionando 2-3 productos reales de ejemplo (mira el inventario real de abajo), y pasa directo a preguntar que anda buscando especificamente. Ejemplo del formato esperado (adaptalo, no lo copies literal):
"¡Claro! Vendemos ropa: remeras, pantalones, chompas, vestidos y más. ¿Qué andás buscando?"` : ` Respondele con las categorias reales en formato de LISTA (numerada o con guiones, una por linea), nunca las menciones todas seguidas en una misma frase de parrafo. Ejemplo del formato esperado:
...
- Cuando hay intencion clara de compra, pedile lo que falte (nombre, direccion de entrega, forma de pago) de forma natural y llama a crear_pedido.
...
${faltantes.length ? `- DATOS QUE FALTAN PARA RECOMENDAR BIEN en la categoria "${categoriaActual.nombre}": ${faltantes.join(', ')}. Esta tienda los marco como obligatorios para esta categoria - preguntalos de a uno, de forma natural, ANTES de mostrar productos con mostrar_productos (salvo que el cliente ya haya dado suficiente para que valga la pena mostrar algo igual). Guardalos con actualizar_datos_lead (los que ya son campo propio como talla/color/marca en su campo de siempre, cualquier otro atributo nuevo como Genero o Uso en el parametro atributosCategoria).` : ''}
...
- TARJETAS: para mostrar cualquier producto SIEMPRE llama a mostrar_productos con sus IDs. Nunca escribas precio o stock como texto plano: eso va en la tarjeta que genera el sistema. Idealmente mostras 2-3 opciones MUY relevantes, no satures con mas solo porque existen.
...
- No se puede crear un pedido sin nombre, direccion de entrega y forma de pago: pedilos de forma natural antes de llamar crear_pedido si faltan.
...
async function ejecutarFuncion(toolCall, contexto, helpers) {
  ...
  if (toolCall.name === 'actualizar_datos_lead') {
    ...
    if (args.categoriaInteres) {
      ...
      const resuelto = contexto.find((p) => p.categoria && coincideTexto(datos.categoriaInteres, p.categoria.nombre));
    ...
    if (args.formaPago) datos.formaPago = args.formaPago.toUpperCase();
    ...
    if (args.estadoConversacion) datos.estadoConversacion = args.estadoConversacion.toUpperCase();
    ...
    if (Array.isArray(args.productosDescartadosIds) && args.productosDescartadosIds.length) {
      const actuales = clienteFinal.productosDescartados || [];
    ...
    if (args.atributosCategoria && typeof args.atributosCategoria === 'object') {
      const actuales = clienteFinal.atributosLead || {};
      const nuevos = {};
  ...
  if (toolCall.name === 'mostrar_productos' || toolCall.name === 'enviar_fotos_producto') {
    ...
    let ids = toolCall.name === 'mostrar_productos'
      ? (args.idsProductos || []).slice(0, MAX_PRODUCTOS_A_MOSTRAR).map(Number)
    ...
    if (leadActual) {
      const candidatos = productosCandidatosAMostrar(contexto, leadActual).map((c) => c.id);
    ...
    // FORZADO EN CODIGO (no alcanza con pedirselo al modelo por texto): si el
    // resultado real de busqueda para lo que el cliente esta pidiendo tiene
    // mas productos de los que el modelo eligio mostrar, se completan aca los
    // que faltan. Sin esto, a veces la IA muestra tarjeta de 1 solo producto
    // y "resuelve" el resto describiendolo en texto plano, rompiendo la regla
    // de que todo dato de producto va en tarjeta.
    if (toolCall.name === 'mostrar_productos' && leadActual) {
      const candidatos = productosCandidatosAMostrar(contexto, leadActual);
      for (const c of candidatos) {
        if (ids.length >= MAX_PRODUCTOS_A_MOSTRAR) break;
        if (!ids.includes(c.id) && !yaEnviadas.includes(c.id)) ids.push(c.id);
      }
    }
    ...
    if (toolCall.name === 'mostrar_productos') {
      ...
      if (ids.length && idsNuevos.length === 0) {
        return 'Estos productos YA se los mostraste en esta conversacion (mira el historial, ya tienen tarjeta). NO vuelvas a llamar mostrar_productos con los mismos IDs: en tu texto respondele en base a lo que ya vio (pregunta cual le gusto, resolvele una duda, o si no hay mas opciones reales en el bloque de resultados, decile con naturalidad que esas son todas las que hay por ahora).';
      }
      ids = idsNuevos;
    }
    ...
    for (const producto of productos) {
      const caption = fichaProducto(producto, leadActual || {});
      ...
      const mediaUrl = fotoParaMostrar(producto, leadActual || {});
      if (conexion && conexion.estado === 'CONECTADO' && mediaUrl) {
        await wa.enviarImagenes(conexion, telefonoCliente, [mediaUrl], caption);
    ...
    const mostradosNuevos = toolCall.name === 'mostrar_productos'
    ...
    logEtapa('tool_ejecutada', telefonoCliente, { tool: toolCall.name, productos: productos.map((p) => p.id) });
  ...
  if (toolCall.name === 'crear_pedido') {
    ...
    if (!clienteFinal.formaPago) {
      return 'TODAVIA NO crees el pedido: falta la forma de pago. Preguntasela (solo entre las formas de pago reales de esta tienda, ver bloque de arriba), guardala con actualizar_datos_lead y recien ahi vuelve a llamar crear_pedido.';
    }
    if (!metodosHabilitados[clienteFinal.formaPago]) {
      ...
      return `TODAVIA NO crees el pedido: esta tienda no tiene habilitada esa forma de pago. Las formas de pago reales son: ${disponibles || 'ninguna configurada, avisale que un asesor lo va a contactar para coordinar el pago'}. Preguntale de nuevo entre esas, guardala con actualizar_datos_lead y recien ahi vuelve a llamar crear_pedido.`;
    }
    ...
    const pedido = await prisma.$transaction(async (tx) => {
      ...
      await tx.clienteFinal.update({ where: { id: clienteFinal.id }, data: { estadoLead: 'PEDIDO_CREADO', estadoConversacion: 'LISTO_PARA_COMPRAR' } });
      return creado;
    });
    logEtapa('pedido_creado', telefonoCliente, { pedidoId: pedido.id, items: items.length, total });
    ...
    return `Pedido #${pedido.id} creado con: ${resumen}.${avisoQr} Tu texto ahora: confirma el pedido al cliente con calidez, y explica que un asesor coordinara la entrega. Si tiene dudas de pago, ofrece derivar_a_asesor.`;
  }

  if (toolCall.name === 'mostrar_catalogo') {
    // El dominio real se toma del propio request que llego (webhook de
    // WhatsApp o chat de prueba): asi el link nunca depende de que alguien
    // haya configurado bien APP_BASE_URL en el servidor. Ese env var queda
    // solo como respaldo para scripts que corren fuera de un request real.
    const base = helpers.baseUrl || process.env.APP_BASE_URL || 'http://localhost:3000';
...
async function generarRespuesta(agenteId, telefonoCliente, historial, mensajeCliente, conversacionId, opciones = {}) {
        ...
        empresa: {
          include: {
            ...
            productos: {
              ...
              orderBy: { nombre: 'asc' },
              take: 200,
              include: { variantes: { where: { activa: true }, orderBy: { id: 'asc' } }, categoria: { include: { atributos: true } } },
            },
          },
        },
      },
  ...
  logEtapa('atributos_detectados', telefonoCliente, { ...detectados, presupuesto: lead.presupuesto, marca: lead.marca, talla: lead.talla });

  const candidatosActuales = productosCandidatosAMostrar(productos, lead);
  logEtapa('busqueda_ejecutada', telefonoCliente, { categoria: lead.categoriaInteres || null, resultados: candidatosActuales.length });
  ...
  const helpers = { empresaId: empresa.id, empresaSlug: empresa.slug, telefonoCliente, conexion: agente.conexion, fotosParaMostrar, conversacionId, baseUrl: opciones.baseUrl, config: agente.config };
  const mensajesTurno = [];
  const MAX_VUELTAS = 3;
...
module.exports = {
  ...
  respuestaErrorTecnico, seccionProductos, fichaProducto,
};
