  ...
  try {
    for (let vuelta = 0; vuelta < MAX_VUELTAS; vuelta++) {
      ...
      if (!resp.tool_calls?.length) {
        ...
        const pidiendoPreferencia = !yaEligio && !helpers.tarjetasEnTurno && candidatosActuales.length && pidePreferenciaSinMostrar(resp.content);
        ...
        if (!esUltimaVuelta) {
          ...
          const faltaInicial = preguntasInicialesFaltantes(agente.config, leadActual)[0];
