...
async function procesarMensajeEntranteInterno({ agenteId, telefonoCliente, contenido }) {
  ...
  if (abierta) {
    ...
    return { ok: true, cobrada: false, conversacionId: abierta.id, origen: abierta.origen };
  }
  ...
  return { ok: true, cobrada: true, conversacionId: conversacion.id, origen: cobro.origen };
}
...
async function generarYRegistrarRespuesta(agenteId, telefonoCliente, conversacionId, contenido) {
  ...
  const salida = await generarRespuesta(agenteId, telefonoCliente, historial, contenido);
